    ...
    public class LongRecoilSystem_ClosedBolt : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (wasHeld)
            {
                ...
                Vector3 lerpPos = Vector3.Lerp(newBoltRearwardPos.localPosition, newBoltForwardPos.localPosition, boltLerp);
                newBolt.transform.localPosition = lerpPos;
                if (originalBolt.CurPos == ClosedBolt.BoltPos.Forward) wasHeld = false;
            }

            if (!wasHeld)
            {
                if (originalBolt.CurPos == ClosedBolt.BoltPos.ForwardToMid && currentZ < lastZ)
                {
                ...
                else if (originalBolt.CurPos == ClosedBolt.BoltPos.Locked && currentZ < lastZ)
                {
                ...
                else if (originalBolt.CurPos == ClosedBolt.BoltPos.LockedToRear && currentZ < lastZ)
                {
                ...
                else if (originalBolt.CurPos == ClosedBolt.BoltPos.Rear && (currentZ < lastZ || currentZ == lastZ))
                {
                    newBolt.transform.localPosition = newBoltRearwardPos.localPosition;
                    barrel.transform.localPosition = barrelForwardPos.localPosition;
                }
                else if ((originalBolt.CurPos == ClosedBolt.BoltPos.LockedToRear || originalBolt.CurPos == ClosedBolt.BoltPos.ForwardToMid) && currentZ > lastZ)
                {
                    float boltLerp = originalBolt.GetBoltLerpBetweenRearAndFore();
                ...
                else if (originalBolt.CurPos == ClosedBolt.BoltPos.Forward && (currentZ > lastZ || currentZ == lastZ))
                {
                ...
                if (!soundPlayed && ((originalBolt.CurPos == ClosedBolt.BoltPos.Rear && (currentZ < lastZ || currentZ == lastZ)) || (originalBolt.CurPos == ClosedBolt.BoltPos.LockedToRear && currentZ > lastZ) || (originalBolt.CurPos == ClosedBolt.BoltPos.ForwardToMid && currentZ > lastZ)))
                {
