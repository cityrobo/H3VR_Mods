...
namespace ShotTimer
{
    public class ShotLineFactory
    {
        ...
        ShotLine prev = new ShotLine(0, 0);
        public ShotLine Create(OverflowArray<ShotLine> shot_database)
        {
            ...
            if (shot_database.Count != 0)
            {

                line = new ShotLine(time - start_time, (time - start_time) - prev.Time);
            }
            else
            {
                line = new ShotLine(time - start_time, time - start_time);
            }
            prev = line;
